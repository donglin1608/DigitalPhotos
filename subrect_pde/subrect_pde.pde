...
// This is the code to generate the image just showed.
...
void  draw()
{
  ...
  if (gen)
  {
    ...
    int it = 32; //Change initailize value to generate different visual
    float rot = 0;
    for (int i=0; i<it; i++) {
      ...
      while (h < tH)
      {
        ...
        while (w < tW)
        {
          ...
          //rectangle(w, h, modw, modh, 0, round, 0, round);
          noStroke();
          beginShape();
          fill(color1);
          vertex(w, h);
...
//This is another function for the noise purpose
